...
这个仓库的目标是把“RL 调 PID + 安全层兜底”这件事做成一个能训练、能演示、能看清作用分工的原型，不是一次性把问题做成复杂仿真平台。

所以当前选择是刻意收敛过的：

- 低维观测，不做高维感知
- 单机器人，不做多车交互
- 简单安全规则，不上 barrier function
- 只调两个高层变量，不做大规模控制参数联动

这不是偷懒，而是为了避免把性能变化混在一堆复杂因素里，最后连“到底是不是 RL 调 PID 起作用”都说不清。
